...
double SwerveModule::encoderAngleRadians() {
 double sensReading = m_absoluteEncoder.GetPosition().GetValueAsDouble();
 double adjReading = fmod(sensReading, 1.0);
 double newReading;

 if(adjReading < 0) {
  newReading = adjReading + 0.5;
 } else {
  newReading = adjReading - 0.5;
 }

 double m_absoluteEncoderRadians = newReading * (std::numbers::pi * 2.0);

 return m_absoluteEncoderRadians;
}
...
/*
  ...
  */
  double m_absoluteEncoderRadians = encoderAngleRadians();
